...
\begin{algorithm}
\KwIn { $Tree(V,E)$ is a $1$-tree and $Tree_1(V,E_1)$ is another tree where $E\cup E_1$ is the set of all possible edges between vertices $V=\{v_1,v_2,.....,v_n\}$ and $E\cap E_1=\emptyset$  }
\KwOut{A partial $2$-tree.}
\nl \While{$(E.size<2*|V|-3)$}
{
\nl select the edge $e$ from $Tree1$ which has highest connectivity probability.\\
\nl \If{check$(Tree(V,E_1\cup e)$}
{
\nl add $e$ to the edgeset $E$ of $Tree$\\
}
\nl remove $e$ from the edgeset $E_1$ of $Tree1$ \\
}
\nl \Return{$Tree$}
\caption{Function TwoTree$(Tree,Tree1)$}
\end{algorithm}

\begin{algorithm} 
\Indm
\KwIn{ a UWSN $Tree=(V,E)$ is a partial $2$-tree where each node, $v\in V$  can be located into a set of regions $R_v=\{r_{(v,1)},r_{(v,2)}...\}$ with probability,  $\{p(r_{(v,1)}),p(r_{(v,2)})...\}$ and $(x,y)\in E$ if $x\in V$ can be located one of it's locality set and $y\in V$ can be located one of it's locality set, so that they reach each other.}
\KwOut{ $flag$ }

%\noline
\Indp
\nl \textbf{Initialization:} set the $counter$ to 0 and $flag$ to $false$.\\
\nl Extract all the nodes from $Tree$ and put into queue, $Q$ .\\
\nl\While{$Q!=Empty$ and $counter<Q.size$}
{
 \nl Remove the front node $n$, form the $Q$\\
\nl find the degree, $deg$ of $n$ in the tree, $Tree$.\\
\nl \If{$deg==1$}{
\nl find and remove edge $e$ from $Tree$ associated with node $n$\\
\nl reset the $counter$
}
\nl \ElseIf{$deg==2$}{
\nl find the edges $e_1$ and $e_2$ associated with node $n$ and neighbour nodes of $n$, $n_1,n_2$ \\
\nl remove $e_1$ and $e_2$ from $Tree$\\ 
\nl if there is no edge between $n_1$ and $n_2$ in $Tree$ then add a edge between them.\\
\nl reset the $counter$
}
 \Else {
\nl add $n$ to the end of the queue,$Q$ and increment the $counter$ by one}
 }
 \If{$Q$ is $empty$}{
\nl set $flag$=$true$
}
 \Else {
\nl set the $flag=false$}
\nl \Return{flag}
\caption{Function Check$(Tree)$}
\end{algorithm}
